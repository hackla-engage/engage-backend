...
from CouncilTag.ingest.models import Message, Committee, EngageUserProfile 
...
def paragraphize_comments(comments, contents):
    ...
    for comment in comments:
        if comment.email is not None:
            if EngageUserProfile.objects.filter(user_id=comment.user_id)[0].verified:
                verified = "[VERIFIED]"
            else:
                verified = ""

            name = "Name: " + comment.first_name + " " + comment.last_name
            email = "Email: <a href='mailto:" + str(comment.user.email) + "'><u>" + comment.user.email + f"</u></a> <strong>{verified}</strong>"
...
def writePdfForAgendaItems(agenda_items):
    ...
    try:
        ...
        cover_page_subtitle_leading = ParagraphStyle("cover", fontSize=12, spaceBefore=50, alignment=TA_CENTER, textColor="#000000")
        ...
        contents.append(Paragraph("Report generated by <a href='http://engage.town/'><u>Engage.town</u></a>", cover_page_subtitle_leading))
